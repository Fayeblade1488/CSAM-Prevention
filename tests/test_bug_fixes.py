...
def test_rate_limiter_thread_safety():
    ...
    allowed = sum(results)
    assert allowed == 5, (
        f"Expected 5 more allowed requests after window expiry, got {allowed}"
    )
...
def test_rate_limiter_memory_cleanup():
    ...
    assert len(limiter.requests) < 10051, (
        f"Memory cleanup should have removed some old entries, got {len(limiter.requests)}"
    )
    ...
    assert len(limiter.requests) < 200, (
        f"Expected much fewer entries after cleanup, got {len(limiter.requests)}"
    )
...
def test_fastapi_lifespan_no_deprecation():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        on_event_warnings = [
            ...
            if "on_event" in str(warning.message)
        ]

        # We expect no on_event deprecation warnings
        assert len(on_event_warnings) == 0, (
            f"Found {len(on_event_warnings)} on_event deprecation warnings"
        )
...
def test_rate_limiter_edge_cases():
    ...
    assert sum(results) == 2, (
        f"Expected 2 allowed in rapid succession, got {sum(results)}"
    )
